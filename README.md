...
Privacy warning: generated packets, CSV exports, casebooks, evidence bundles, and checksum manifests can contain personal balances, income, expenses, event labels, and scenario assumptions. Keep user-generated outputs local unless you have reviewed and intentionally sanitized them for sharing.
...
- `release-check`: validate expected package/docs/demo files, public scan status, and generated HTML safety requirements.
...
- `csv-export`: export packet assets, runway rows, warnings, and bucket summaries into deterministic spreadsheet-hardened CSV plus a manifest.
...
- `public-scan`: scan a repo tree for public-release concerns, including absolute local path leakage, provider-token patterns, secret-like environment labels, and unsafe generated HTML.
