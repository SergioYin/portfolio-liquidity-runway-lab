...
- Wheel-install smoke documentation: build with `python -m build`, create a clean virtual environment, install the generated wheel with `python -m pip install dist/portfolio_liquidity_runway_lab-0.10.1-py3-none-any.whl`, then run `portfolio-liquidity-runway-lab selfcheck`, `portfolio-liquidity-runway-lab template-pack --out release-smoke/templates`, and `portfolio-liquidity-runway-lab quickstart-check --out release-smoke/quickstart`.
